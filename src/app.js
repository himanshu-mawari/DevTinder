...
4 Build the Cloudinary upload service (if not already present)
  A function that takes req.file.buffer, streams it to Cloudinary (use upload_stream since you have a buffer, not a file path — cloudinary.uploader.upload expects a path/base64, upload_stream expects a buffer), and returns a Promise resolving to the result (you need result.secure_url). Commit: 'feat: add cloudinary buffer upload service'
